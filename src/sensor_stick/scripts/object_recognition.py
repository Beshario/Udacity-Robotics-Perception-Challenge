...
def pcl_callback(pcl_msg):
    ...
    # PassThrough Filter to crop the pic along the z axis and isolate the table and objects
    ...
    seg.set_model_type(pcl.SACMODEL_PLANE) #plane model
    seg.set_method_type(pcl.SAC_RANSAC) #Random sampling consensus method (to cluster a plane) 
    max_distance = 0.01
    ...
    # Convert the xyzrgb cloud to only xyz since k-d tree only needs xyz #K-d tree
    ...
    for index, pts_list in enumerate(cluster_indices):
        ...
        # Extract histogram features (sisame as capture_features.py)
        histogram_bins = 64
